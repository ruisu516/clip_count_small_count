...
class CrowdCLIP(nn.Module):
    def __init__(
        ...
        if kwargs:
            print(f"irrelevant kwargs: {kwargs}")
        ...
        # convert to float32
        clip_model.float()
        print("convert `clip_model` to float32. if need fp16 model, call `clip.model.convert_weights`")
...
def load_clip_to_cpu(
    ...
    if image_backbone_name != text_backbone_name:
        ...
        if MODEL is None:
            ...
            print(f"Try PyTorch Official image model: {image_backbone_name}")
        else:
            print(f"Try Custom image model: {image_backbone_name}")
        if MODEL is None:
